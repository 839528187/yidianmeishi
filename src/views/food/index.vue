<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button size="small" type="primary" plain @click="createFood()">添加食材</el-button>
      <el-input v-model="listQuery.keywords" clearable size="small" placeholder="请输入食材标题" style="width: 200px; margin-left: 10px; margin-bottom:1px;" class="filter-item" @blur="getList" @clear="getList"/>
    ...
    <el-table
      ...
      <el-table-column label="操作" prop="status" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-row>
            <el-button type="text" circle @click="updateFood(scope.row.id)">编辑</el-button>
...
<script>
import { list, deletes } from '@/api/food'
export default {
  data() {
    return {
      tableKey: 0,
  ...
  methods: {
    ...
    // 删除食物相克
    changeDelete(id) {
      this.$confirm('此操作将删除该食材相克吗, 是否继续?', '提示', {
    ...
    // 跳转到添加新闻
    createFood() {
      this.$router.push('/food/operation')
    },

    // 跳转到编辑新闻
    updateFood(id) {
      this.$router.push('/food/operation/' + id)
    },
